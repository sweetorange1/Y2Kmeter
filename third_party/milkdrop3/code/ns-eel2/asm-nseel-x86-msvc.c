...
#else  // !_M_IX86 — x64 / ARM: MSVC 不支持 __asm / __declspec(naked)
       // 提供空实现 stub，EEL2 编译器检测到代码大小为 0 后回退到解释模式。

// 声明所有 nseel_asm_* 符号为外部引用（由 nseel-compiler.c 的 fnTable1[] 使用）
#define DECL_STUB(x)          \
...
// 额外符号（fnTable1 中引用的非 DECL_ASMFUNC 函数）
...
// win64_callcode —— x64 stub（EEL2 在 stub 模式下代码大小为 0，永不实际调用此函数）
// 注意: 使用 void* 而非 INT_PTR，因为本文件不包含 <windows.h>，INT_PTR 不可用。
void win64_callcode(void* code) { (void)code; }
