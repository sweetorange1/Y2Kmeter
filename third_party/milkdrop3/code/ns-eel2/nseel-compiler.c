...
// x64: 所有 nseel_asm_* 函数均为空 stub（DECL_STUB 宏），
// 不含 ~(INT_PTR)0 哨兵值 → 无需 JIT 立即数补丁。
// 直接返回原指针，避免扫描未初始化/跨页内存导致 Debug 模式下挂起。
INT_PTR *EEL_GLUE_set_immediate(void *_p, void *newv)
{
  (void)newv;
  return (INT_PTR*)_p;
...
{
  char *p=(char*)_p;
  const char* const p_end = p + 4096;  // x86: 设上限防止异常数据导致无限扫描
  while (p < p_end && *(INT_PTR *)p != ~(INT_PTR)0) p++;
  if (p >= p_end) return ((INT_PTR*)_p);  // fallback: 返回原指针
...
static void *GLUE_realAddress(void *fn, void *fn_e, int *size)
{
  ...
  p=(unsigned char *)fn;
  {
    const size_t kMaxScan = 4096;  // x64 上 GLUE_RET 为单字节 0xC3，
                                   // 永远不会匹配到 12 字节 0x89+NOP 模板；
                                   // 设上限防止 p 越界导致死循环/访问违例。
    const unsigned char* const pEnd = p + kMaxScan;
    for (;;)
    {
      int a;
      for (a=0; a<12; a++)
  ...
  // 未找到预期字节序列 → 返回函数起始地址，size=0
  // （x64 上 GLUE 生成代码的 ret 布局与 x86 不同，此路径为预期路径）
  *size = 0;
  return fn;
#else
